...
import { AlertTriangle, ArrowRight, Bot, Brain, Building2, Check, CheckCircle2, Code2, FileText, MessageSquare, Mic, Menu, X, Play, Radar, Sparkles, Terminal, Upload, Users, Video, Wand2, Wifi, Zap } from "lucide-react";
...
export default function Landing() {
  return (
    <main className="dark min-h-screen bg-background text-foreground overflow-x-hidden" style={{ background: "var(--background)", color: "var(--foreground)" }}>
        ...
        <div className="glass-strong rounded-none sm:rounded-2xl px-4 py-3 shadow-card">
          <div className="flex items-center justify-between">
            ...
            <div className="hidden items-center gap-2 md:flex">
              <Link to="/register" className="text-sm text-muted-foreground hover:text-foreground">Sign in</Link>
              <a
                href="#cta"
            ...
            {/* Mobile Menu Button */}
            <button className="block md:hidden p-1 text-muted-foreground hover:text-foreground" onClick={() => setIsOpen(!isOpen)}>
          ...
          {isOpen && (
            <div className="mt-4 flex flex-col gap-4 border-t border-border/40 pt-4 pb-2 md:hidden">
              {links.map((l) => (
                <a key={l.label} href={l.href} onClick={() => setIsOpen(false)} className="text-sm text-muted-foreground hover:text-foreground transition-colors">
                  {l.label}
                </a>
              ))}
              <hr className="border-border/40" />
              <div className="flex flex-col gap-3">
                <Link to="/register" onClick={() => setIsOpen(false)} className="text-center text-sm text-muted-foreground hover:text-foreground py-1">
                  Sign in
                </Link>
                <a
                  href="#cta"
      ...
      <div className="relative mx-auto max-w-6xl px-4">
        <div className="mx-auto max-w-3xl text-center">
          <div className="mx-auto inline-flex items-center gap-2 rounded-full border border-border bg-secondary/40 px-3 py-1 text-[11px] sm:text-xs text-muted-foreground backdrop-blur max-w-full overflow-hidden text-ellipsis">
            ...
            <span className="truncate">Powered by Groq · Llama 3.1 · Real-time inference</span>
          </div>
          ...
          <div className="mt-8 flex flex-col sm:flex-row flex-wrap items-center justify-center gap-3 px-4 sm:px-0">
            <a href="#cta" className="group w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-xl bg-gradient-brand px-5 py-3 text-sm font-medium text-primary-foreground shadow-glow transition-transform hover:scale-[1.02]">
              Start Mock Interview
              <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-0.5" />
            </a>
            <a href="#preview" className="w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-xl border border-border bg-secondary/50 px-5 py-3 text-sm font-medium text-foreground backdrop-blur transition-colors hover:bg-secondary">
            ...
            <div className="grid grid-cols-1 gap-3 rounded-xl bg-background/60 p-2 sm:p-3 lg:grid-cols-12">
              ...
              <div className="lg:col-span-5 rounded-xl border border-border bg-card p-4 flex flex-col justify-between">
                <div>
                  ...
                  <div className="mt-4 space-y-3 text-sm">
                    ...
                    <div className="rounded-lg bg-gradient-brand/10 p-3">
                      ...
                      <p className="mt-1 text-foreground/90 text-xs sm:text-sm">I'd use base62 encoding over an auto-incrementing ID, shard by hash prefix, and front the read path with a Redis LRU cache...</p>
                    </div>
                  </div>
                </div>
                <div className="mt-4 flex items-center justify-between border-t border-border/40 pt-4 lg:border-0 lg:pt-0">
                  ...
                  <span className="text-[11px] text-muted-foreground font-mono">02:14 / 45:00</span>
                </div>
              </div>
              ...
              <div className="lg:col-span-7 rounded-xl border border-border bg-card p-4">
                ...
                <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
                  {[
                    ...
                    <div key={m.k} className="rounded-lg border border-border bg-background/50 p-2.5 sm:p-3">
                      <div className="text-[10px] sm:text-[11px] text-muted-foreground truncate">{m.k}</div>
                      <div className="mt-0.5 text-xl sm:text-2xl font-semibold tabular-nums">{m.v}</div>
                ...
                <div className="mt-4 space-y-2 text-xs">
                  <div className="flex items-start gap-2 rounded-lg bg-success/10 p-2.5 text-success">
                    <span className="font-bold">✓</span> <p>Strong tradeoff reasoning on cache eviction.</p>
                  </div>
                  <div className="flex items-start gap-2 rounded-lg bg-warning/10 p-2.5 text-warning">
                    <span className="font-bold">!</span> <p>Missed: discuss collision handling for hash truncation.</p>
                  </div>
                  <div className="flex items-start gap-2 rounded-lg bg-secondary/50 p-2.5 text-muted-foreground">
                    <span className="font-bold">›</span> <p>Suggest adding back-of-envelope: 10K writes/sec (~10 TB/year).</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="mt-16 sm:mt-20">
          ...
          <div className="mt-5 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4 items-center justify-center opacity-60">
            {["Google", "Amazon", "Meta", "Stripe", "Microsoft", "TCS"].map((n) => (
              <div key={n} className="text-center text-sm font-semibold tracking-tight text-muted-foreground py-2 border border-border/20 rounded-xl bg-secondary/10 sm:border-0 sm:bg-transparent">{n}</div>
            ))}
          </div>
        </div>
      </div>
            ...
            <div className="glass rounded-2xl p-4 sm:p-5 shadow-card overflow-x-auto">
              <div className="flex items-center justify-between min-w-[280px]">
                <div className="flex items-center gap-2 text-xs sm:text-sm">
                  <Sparkles className="h-4 w-4 text-brand" /> Evaluation · Two Sum
                </div>
                <span className="rounded-full bg-success/15 px-2 py-0.5 text-[10px] text-success">Score 92</span>
              </div>
              <pre className="mt-4 overflow-x-auto rounded-lg bg-background/70 p-3 text-[11px] sm:text-[12px] leading-relaxed text-foreground/90 font-mono"><code>{`function twoSum(nums, target) {
  const seen = new Map();
  for (let i = 0; i < nums.length; i++) {
    ...
    seen.set(nums[i], i);
              ...
              <div className="mt-4 space-y-2 text-xs min-w-[280px]">
                <div className="flex items-start gap-2 rounded-lg bg-success/10 p-2.5 text-success">
                  <CheckCircle2 className="mt-0.5 h-3.5 w-3.5 flex-shrink-0" /> <span>Optimal O(n) single-pass. Clear naming.</span>
                </div>
                <div className="flex items-start gap-2 rounded-lg bg-warning/10 p-2.5 text-warning">
                  <AlertTriangle className="mt-0.5 h-3.5 w-3.5 flex-shrink-0" /> <span>Add explicit fallback return case.</span>
                </div>
              </div>
            </div>
          </div>
    ...
    <section id="coding" className="relative py-16 sm:py-28">
      ...
      <div className="mx-auto max-w-6xl px-4">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <p className="mt-4 text-sm sm:text-base text-muted-foreground">
            Monaco editor + Judge0 CE backend. 40+ languages, hidden test cases, complexity hints, and AI-assisted support.
          </p>
        </div>
        ...
        <div className="mt-12 overflow-hidden rounded-2xl border border-border bg-card shadow-card">
          <div className="flex items-center justify-between border-b border-border bg-background/60 px-4 py-2.5 gap-2">
            <div className="flex items-center gap-1 sm:gap-2 text-xs overflow-x-auto scrollbar-none whitespace-nowrap py-1">
              ...
              <span className="rounded-md px-2 py-1 text-muted-foreground opacity-60 sm:opacity-100">binary-tree.ts</span>
              <span className="rounded-md px-2 py-1 text-muted-foreground hidden sm:inline">lru-cache.ts</span>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12">
            <aside className="border-b lg:border-b-0 lg:border-r border-border bg-background/40 p-4 text-xs lg:col-span-3">
              ...
              <p className="mt-2.5 text-muted-foreground leading-relaxed">Given an array of integers <code className="rounded bg-secondary/60 px-1 font-mono">nums</code> and target value, return index pairs.</p>
              <div className="mt-4 hidden sm:block rounded-md bg-secondary/40 p-2 font-mono">
                ...
                <code className="text-foreground text-[11px]">nums = [2,7,11], target = 9 -&gt; [0,1]</code>
              </div>
            </aside>
...
function twoSum(nums: number[], target: number): number[] {
  ...
  return [];
            ...
            <aside className="border-t lg:border-t-0 lg:border-l border-border bg-background/40 p-4 text-xs lg:col-span-3 space-y-4">
              <div>
                <div className="text-[11px] uppercase tracking-wider text-muted-foreground mb-2">Test cases</div>
                <ul className="space-y-1.5 font-mono text-[11px]">
                  <li className="flex items-center justify-between rounded-md bg-secondary/40 px-2 py-1.5">
                    <span className="truncate text-muted-foreground">[2,7,11,15], 9</span>
                    <CheckCircle2 className="h-3.5 w-3.5 text-success flex-shrink-0 ml-1" />
                  </li>
                  <li className="flex items-center justify-between rounded-md bg-secondary/40 px-2 py-1.5">
                    <span className="truncate text-muted-foreground">[3,2,4], 6</span>
                    <CheckCircle2 className="h-3.5 w-3.5 text-success flex-shrink-0 ml-1" />
                  </li>
                </ul>
              </div>
      ...
      <div className="mx-auto max-w-6xl px-4">
        <div className="grid items-center gap-12 lg:grid-cols-2">
          <div className="relative order-2 lg:order-1 w-full overflow-hidden">
            ...
            <div className="glass rounded-2xl p-4 sm:p-6 shadow-card">
              ...
              <div className="mt-4 flex items-center justify-center overflow-x-auto py-2">
                <svg viewBox="0 0 320 320" className="h-[280px] w-[280px] sm:h-[320px] sm:w-[320px] flex-shrink-0">
                  ...
                  {axes.map((label, i) => {
                    ...
                    return (
                      <text key={label} x={x} y={y} textAnchor="middle" dominantBaseline="middle" className="fill-muted-foreground font-medium" style={{ fontSize: 10 }}>
                        {label}
                      </text>
                    );
                  })}
              ...
              <div className="mt-4 grid grid-cols-3 gap-2 text-center text-xs">
                ...
                <div className="rounded-lg border border-border bg-background/40 p-2 sm:p-3">
                  ...
                  <div className="text-base sm:text-xl font-semibold">17d 🔥</div>
                </div>
                <div className="rounded-lg border border-border bg-background/40 p-2 sm:p-3">
                  <div className="text-[10px] sm:text-xs text-muted-foreground">Progress</div>
                  <div className="text-base sm:text-xl font-semibold text-success">+18%</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="order-1 lg:order-2">
            ...
            <p className="mt-4 text-sm sm:text-base text-muted-foreground">
              The performance radar maps your strengths against standard corporate role configurations — tracking weak points directly.
            </p>
      ...
      <div className="mx-auto max-w-6xl px-4">
        <div className="grid items-center gap-12 lg:grid-cols-2">
          <div>
            ...
            <p className="mt-4 text-sm sm:text-base text-muted-foreground">
              We parse every project and key phrase — then generate targeted system questions to avoid live-interview surprises.
            </p>
            <div className="mt-6 grid grid-cols-3 gap-2 text-center text-sm">
              {[
                ...
                <div key={s.l} className="rounded-xl border border-border bg-card p-2 sm:p-4">
                  <div className="text-xl sm:text-2xl font-semibold">{s.n}</div>
                  <div className="text-[10px] sm:text-xs text-muted-foreground truncate">{s.l}</div>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="glass rounded-2xl p-4 sm:p-5 shadow-card">
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 rounded-xl border border-dashed border-border bg-background/40 p-4 w-full">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium truncate">arjun_resume.pdf</div>
              ...
              <div className="mt-5 space-y-3">
                <div className="text-[11px] uppercase tracking-wider text-muted-foreground inline-flex items-center gap-1.5">
                  <Wand2 className="h-3.5 w-3.5 text-brand" /> Generated deep dives
                </div>
                {[
                  ["Distributed cache project", "Walk me through your system architecture and error state controls."],
                  ["React performance work", "How did you scale interface calculations safely?"],
                ].map(([tag, q]) => (
                  <div key={q} className="rounded-xl border border-border bg-card p-3.5">
                    <div className="text-[10px] uppercase tracking-wider text-brand font-mono">{tag}</div>
                    <div className="mt-1 text-xs sm:text-sm text-foreground/90 leading-relaxed">{q}</div>
                  </div>
                ))}
              </div>
            </div>
    ...
    <section id="companies" className="relative py-16 sm:py-28">
      <div className="mx-auto max-w-6xl px-4">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <p className="mt-4 text-sm sm:text-base text-muted-foreground">
            Curated target pipelines built off structured historic rubric matrices.
          </p>
        </div>
            ...
            <div key={c.name} className="group relative overflow-hidden rounded-2xl border border-border bg-card p-5 transition-transform hover:-translate-y-0.5">
              ...
              <div className="relative">
                ...
                <div className="mt-5 flex items-center justify-between text-xs border-t border-border/20 pt-4">
                  <span className="text-muted-foreground text-[11px]">240+ problems available</span>
                  <span className="text-brand font-medium group-hover:translate-x-1 transition-transform">Practice &rarr;</span>
                </div>
              </div>
            </div>
          ))}
          ...
          <div className="relative w-full overflow-hidden">
            ...
            <div className="glass rounded-2xl p-4 sm:p-5 shadow-card">
              <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border/40 pb-3 mb-3">
                ...
                <span className="text-[11px] text-muted-foreground font-mono">45:00 left</span>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {[
                  ...
                  <div key={p.name} className="relative aspect-video overflow-hidden rounded-xl border border-border bg-gradient-to-br bg-secondary/20">
                    <div className="absolute inset-0 grid place-items-center">
                      <Video className="h-7 w-7 text-foreground/40" />
                    </div>
                    <div className="absolute bottom-2 left-2 rounded bg-background/80 px-2 py-0.5 text-[10px] font-medium">{p.name}</div>
                  </div>
                ))}
              </div>
...
- Real-time sliding configuration`}</pre>
          ...
          <div>
            ...
            <p className="mt-4 text-sm sm:text-base text-muted-foreground">
              WebSocket rooms containing shared Monaco dynamic state, direct synchronization rules, and immediate template support.
            </p>
            <div className="mt-6 grid grid-cols-2 gap-3 text-sm">
              {[
                ...
                const I = Icon;
                return (
                  <div key={t} className="rounded-xl border border-border bg-card p-3 sm:p-4">
                    <I className="h-4 w-4 text-brand" />
                    <div className="mt-2 font-medium text-xs sm:text-sm">{t}</div>
                    <div className="mt-0.5 text-[11px] sm:text-xs text-muted-foreground leading-tight">{d}</div>
                  </div>
                );
              })}
            </div>
...
// ===================== Testimonials =====================
const items = [
  { name: "Arjun M.", role: "SDE I @ Amazon", quote: "Did 28 mock rounds on LoopMock. The behavioral feedback was scarily accurate." },
  ...
  { name: "Ravi S.", role: "TCS Digital", quote: "Aptitude + HR mocks were spot on. Cleared TCS NQT with the highest band." },
];
    ...
    <section className="relative py-16 sm:py-28">
      <div className="mx-auto max-w-6xl px-4">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <h2 className="mt-4 text-3xl font-semibold tracking-tight sm:text-4xl md:text-5xl">
            Engineers who <span className="text-gradient-brand">got the job</span>
          </h2>
        </div>
        <div className="mt-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {items.map((t) => (
            <figure key={t.name} className="rounded-2xl border border-border bg-card p-5 flex flex-col justify-between">
...
const tiers = [
  {
    ...
    price: "$0",
    sub: "Forever",
    features: ["3 AI mock interviews / month", "Basic feedback report", "DSA problem library"],
    cta: "Start free",
    highlight: false,
  },
  {
    ...
    <section id="pricing" className="relative py-16 sm:py-28">
      <div className="mx-auto max-w-6xl px-4">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <p className="mt-4 text-sm text-muted-foreground">Cancel anytime. Student discounts available.</p>
        </div>
                ...
                <ul className="mt-6 space-y-3 text-sm border-t border-border/20 pt-4">
                  {t.features.map((f) => (
                    <li key={f} className="flex items-start gap-2">
                      <Check className="mt-0.5 h-4 w-4 text-success flex-shrink-0" /> <span className="text-foreground/90">{f}</span>
                    </li>
                  ))}
                </ul>
              </div>
...
const faqs = [
  ...
  ["Do you store my voice or video?", "Voice is transcribed in real-time and discarded by default. Peer video is fully peer-to-peer via WebRTC."],
];
    ...
    <section id="faq" className="relative py-16 sm:py-28">
      <div className="mx-auto max-w-3xl px-4">
        ...
        <div className="mt-10 space-y-3">
          {faqs.map(([q, a]) => (
            <details key={q} className="group rounded-xl border border-border bg-card p-4 sm:p-5 open:bg-card transition-all">
              <summary className="flex cursor-pointer list-none items-center justify-between text-sm sm:text-base font-medium select-none">
                ...
                <span className="text-muted-foreground transition-transform duration-200 group-open:rotate-45 flex-shrink-0">＋</span>
              </summary>
              <p className="mt-3 text-sm text-muted-foreground leading-relaxed border-t border-border/20 pt-3">{a}</p>
            </details>
          ))}
        </div>
      </div>
      ...
      <div className="mx-auto max-w-5xl px-2 sm:px-4">
        <div className="relative overflow-hidden rounded-2xl sm:rounded-3xl border border-border bg-card p-8 text-center shadow-card sm:p-16">
          ...
          <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-3 w-full max-w-md mx-auto sm:max-w-none">
            <Link to="/register" className="group w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-xl bg-gradient-brand px-5 py-3 text-sm font-medium text-primary-foreground shadow-glow transition-transform hover:scale-[1.02]">
              Start Mock Interview
              <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-0.5" />
            </Link>
            <a href="#pricing" className="w-full sm:w-auto text-center rounded-xl border border-border bg-secondary/50 px-5 py-3 text-sm font-medium backdrop-blur transition-colors hover:bg-secondary">
              View pricing
            </a>
          </div>
        </div>
...
function Footer() {
  const cols = [
    { title: "Product", links: ["AI Interviews", "Coding Env", "Skill Radar", "Resume Analyzer"] },
    ...
    <footer className="relative border-t border-border py-12 sm:py-14">
      <div className="mx-auto max-w-6xl px-4">
        ...
        <div className="mt-12 flex flex-col sm:flex-row items-center justify-between gap-y-3 border-t border-border pt-6 text-[11px] sm:text-xs text-muted-foreground text-center sm:text-left">
          <span>&copy; {new Date().getFullYear()} LoopMock Inc. All rights reserved.</span>
          <span className="opacity-80">Built with React · Groq · Judge0 · Socket.io</span>
        </div>
      </div>
    </footer>
  );
