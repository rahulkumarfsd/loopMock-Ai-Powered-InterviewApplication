...
const { executeCode, getRuntimes, testConnectivity } = require('../services/judge0ce.service');
...
const getProblems = asyncHandler(async (req, res) => {
  ...
  problems.sort((a, b) => (ORDER[a.difficulty] || 2) - (ORDER[b.difficulty] || 2));
...
const getProblem = asyncHandler(async (req, res, next) => {
  const problem = await Question.findById(req.params.id)
    .select('-sampleAnswer -coding.testCases.expectedOutput');
  if (!problem) return next(new ApiError(404, 'Problem not found'));
  sendSuccess(res, 200, 'Problem fetched', { problem });
});

// @route  POST /api/coding/run  (visible test cases only)
const runCode = asyncHandler(async (req, res, next) => {
  ...
  // Run sequentially to avoid overwhelming Judge0 free tier
  const results = [];
...
// @route  POST /api/coding/submit  (all test cases including hidden)
const submitCode = asyncHandler(async (req, res, next) => {
  ...
  if (allCases.length === 0) return next(new ApiError(400, 'No test cases for this problem'));
  ...
  const maxTime   = Math.max(...results.map((r) => parseFloat(r.time) || 0)).toFixed(3);
