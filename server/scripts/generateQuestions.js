 ...
 * Generates 200+ real interview questions using Groq
 *
...
const PLAN = [
  ...
  { type: 'dsa',           difficulty: 'medium', topic: 'arrays and two pointers',    count: 8 },
  { type: 'dsa',           difficulty: 'medium', topic: 'trees and binary search',    count: 7 },
  ...
  { type: 'system-design', difficulty: 'medium', topic: 'scalable web systems',       count: 5 },
  { type: 'system-design', difficulty: 'hard',   topic: 'distributed systems',        count: 5 },
  { type: 'behavioral',    difficulty: 'easy',   topic: 'teamwork communication',     count: 6 },
  { type: 'behavioral',    difficulty: 'medium', topic: 'leadership ownership',       count: 6 },
  { type: 'behavioral',    difficulty: 'medium', topic: 'conflict prioritization',    count: 5 },
  { type: 'frontend',      difficulty: 'easy',   topic: 'HTML CSS',                   count: 5 },
  { type: 'frontend',      difficulty: 'medium', topic: 'JavaScript advanced',        count: 6 },
  { type: 'frontend',      difficulty: 'medium', topic: 'React hooks state',          count: 6 },
  { type: 'backend',       difficulty: 'easy',   topic: 'REST APIs HTTP',             count: 5 },
  { type: 'backend',       difficulty: 'medium', topic: 'databases SQL NoSQL',        count: 6 },
  { type: 'backend',       difficulty: 'medium', topic: 'authentication security',    count: 5 },
  { type: 'backend',       difficulty: 'hard',   topic: 'system architecture Nodejs', count: 4 },
];

const COMPANIES = {
  dsa:            ['google', 'amazon', 'facebook', 'microsoft', 'tcs', 'infosys', 'flipkart'],
...
const saveBatch = async (questions, { type, difficulty, topic }) => {
  const cos  = (COMPANIES[type] || []).slice(0, 3 + Math.floor(Math.random() * 2));
...
const run = async () => {
  try {
    ...
    if (clear) { await Question.deleteMany({}); console.log('🗑️  Cleared\n'); }

    const plan = typeArg ? PLAN.filter((p) => p.type === typeArg) : PLAN;
    let generated = 0, failed = 0;

    for (const batch of plan) {
      process.stdout.write(`  ${batch.type.padEnd(15)} ${batch.difficulty.padEnd(8)} ${batch.topic.padEnd(30)} `);
      try {
