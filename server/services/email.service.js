...
const h1 = (t) =>
  `<h1 style="color:#fff;font-size:22px;margin:0 0 8px">${t}</h1>`;
const p = (t) =>
  `<p style="color:#c0c0cc;font-size:14px;line-height:1.7;margin:12px 0">${t}</p>`;
...
const sendWelcome = (user) =>
  send({
    to: user.email,
    subject: "Welcome to LoopMock! 🎉",
    html: templates.welcome(user.name),
  });
const sendWeeklyReport = (user, stats) =>
  send({
    to: user.email,
    subject: "Your weekly LoopMock report 📊",
    html: templates.weeklyReport(user.name, stats),
  });
const sendStreakReminder = (user, days) =>
  send({
    to: user.email,
    subject: `Don't break your ${days}-day streak 🔥`,
    html: templates.streakReminder(user.name, days),
  });
const sendVerifyEmail = (user, token) =>
  send({
    to: user.email,
    subject: "Verify your LoopMock email",
    html: templates.verifyEmail(user.name, token),
  });
